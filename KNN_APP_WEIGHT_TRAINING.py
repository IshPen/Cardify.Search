...
GLOBAL_LITERAL_DISTANCE_EXP = 1.3
GLOBAL_COSINE_DISTANCE_EXP = 1.0
...
def calc_literal_distance(lemmatized_search, search, phrase):
    ...
    global GLOBAL_LITERAL_DISTANCE_EXP
    ...
    for lemmed_term in lemmatized_search:
        ...
        if lemmed_term in normalized_phrase:
            # print(f'Using Literal_Dist Exp')
            dist += len(lemmed_term)**GLOBAL_LITERAL_DISTANCE_EXP
            # print(lemmed_term)
...
def find_best_match_with_lemming_knn(search_phrase, database_phrases, embeddings, n_neighbors=3):
    global GLOBAL_COSINE_DISTANCE_EXP, GLOBAL_LITERAL_DISTANCE_EXP
    ...
    normalized_phrases = []
    literal_distances = []
    print(f"Lemmatized Split Search: {lemmatized_split_search}")
    ...
    # Exponentially Edit Cosine Similarities
    cosine_similarities = [cosine_similarity**GLOBAL_COSINE_DISTANCE_EXP for cosine_similarity in cosine_similarities]
    literal_distances = [literal_distance**GLOBAL_LITERAL_DISTANCE_EXP for literal_distance in literal_distances]
    ...
    fig.update_layout(
        ...
        yaxis_title="Literal Similarity",
        title="Semantic and Literal Similarity",
...
def on_submit_scaler_changes():
    global change_cosine, GLOBAL_COSINE_DISTANCE_EXP, change_literal, GLOBAL_LITERAL_DISTANCE_EXP
    print(f'--------------'
          f'OldCosGlobal:"{GLOBAL_COSINE_DISTANCE_EXP}, OldLitGlobal: {GLOBAL_LITERAL_DISTANCE_EXP}'
          f'--------------')

    if change_cosine and not correct_top_three_guess:
        GLOBAL_COSINE_DISTANCE_EXP += 0.2
    if change_literal:
        GLOBAL_LITERAL_DISTANCE_EXP += 0.1
    print(f'--------------'
          f'CosGlobal:"{GLOBAL_COSINE_DISTANCE_EXP}, LitGlobal: {GLOBAL_LITERAL_DISTANCE_EXP}'
          f'--------------')


def reset_toggles():
    st.session_state.cttg = False
    st.session_state.cc = False
    st.session_state.cl = False
...
if search_query != "":
    ...
    print(f"Top {5} neighbors: {top_matches}")


    # Show button to confirm whether top return was correct.
    correct_top_three_guess = st.toggle("Was the desired phrase in top 3?", value=False, key="cttg")
    change_cosine = st.toggle("Was actual cosine higher than desired cosine?", value=False, key="cc")
    change_literal = st.toggle("Was actual literal higher than desired literal?", value=False, key="cl")
    submit_changes = st.button("Click to submit new CC and CL values", on_click=on_submit_scaler_changes)
